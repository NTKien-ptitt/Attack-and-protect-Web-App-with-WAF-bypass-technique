...
// Các truy vấn cơ sở dữ liệu và đếm
...
// Đường dẫn file thongke.txt
$file_path = 'thongke.txt';

// Thời gian hiện tại
$current_time = time();
$last_saved_time = file_exists($file_path) ? file_get_contents($file_path . '.last') : 0;

// Kiểm tra nếu đã qua 24 giờ (86400 giây)
if (($current_time - $last_saved_time) > 1) {
    // Lưu thống kê vào file
    $stats = "Thống kê thời gian: " . date('Y-m-d H:i:s') . "\n";
    $stats .= "Số người dùng: " . $usercount . "\n";
    $stats .= "Số admin: " . $admin_count . "\n";
    $stats .= "Số sách: " . $bookscount . "\n";
    $stats .= "Số đơn hàng: " . $orders_count . "\n";
    $stats .= "Số tin nhắn: " . $msgcount . "\n";
    
    file_put_contents($file_path, $stats);
    file_put_contents($file_path . '.last', $current_time); // Lưu thời gian
}
?>
